...
def build_client(cfg: Config, app_factory: Callable[[Config, Any], Any] | None = None) -> tuple[AdoClient, TokenProvider | None]:
    ...
    return AdoClient(cfg.org_url, cfg.project, RequestsTransport(
        token_provider=auth.token, on_unauthorized=auth.invalidate, refresh_token=auth.force_refresh)), auth
