...
- Produces: an installable plugin. `https://github.com/ShariniN/Bug-ADO.git` below is the git URL of this repo once pushed; set it in both `.mcp.json` and `team.toml` `tool.repo_url` in Task 16.
...
```json
{
  "mcpServers": {
    "ado-rca": {
      "command": "uvx",
      "args": ["--from", "git+https://github.com/ShariniN/Bug-ADO.git", "rca-mcp"]
    }
  }
}
```
...
**Files:**
- Modify: `plugin/.mcp.json`, `src/rca_core/defaults/team.toml` (fill `https://github.com/ShariniN/Bug-ADO.git` / `tool.repo_url` once the repo is pushed)
...
After the repo is pushed to Azure Repos, replace `https://github.com/ShariniN/Bug-ADO.git` in `plugin/.mcp.json` and README, set `tool.repo_url` in `team.toml`, then:
...
**Placeholder scan**: `https://github.com/ShariniN/Bug-ADO.git` is intentionally deferred to Task 16 step 3 because the repo has no remote yet; no other placeholders.
