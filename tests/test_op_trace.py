...
def routes_with_culprit_pr(query_ids=(7,), refs=(("release/9.5", "r95"), ("release/10.1", "r101"))):
    ...
    r[("GET", "/pullrequests/7?")] = {"pullRequestId": 7, "title": "Make b nullable", "status": "completed",
                                      ...
                                      "lastMergeSourceCommit": {"commitId": C2}, "lastMergeTargetCommit": {"commitId": C1}}
    ...
    r.update(diff_route(R, C1, C2, [("pay.py", "edit", None)]))
    return r
...
def test_trace_finds_culprit_pr_chain_and_versions(tmp_path):
    ...
    assert out["test_signal"] == {"fix_touched_tests": False, "culprit_pr_touched_tests": False, "test_paths": []}
...
def test_trace_without_pr_adds_note(tmp_path):
    ...
    assert out["test_signal"]["culprit_pr_touched_tests"] is None
...
def test_trace_signal_fix_touched_tests_true(tmp_path):
    cfg = make_cfg(tmp_path)
    r = routes_with_culprit_pr()
    r.update(diff_route(R, BASE, HEAD, [("pay.py", "edit", None), ("tests/test_pay.py", "edit", None)]))
    r[("GET", f"/repositories/{R}/commits?searchCriteria.itemPath=tests/test_pay.py")] = {"value": []}
    r[("TEXT", f"items?path=tests/test_pay.py&versionDescriptor.version={BASE}")] = "def test_calc():\n    assert calc(1) == 1\n"
    r[("TEXT", f"items?path=tests/test_pay.py&versionDescriptor.version={HEAD}")] = "def test_calc():\n    assert calc(1) == 2\n"
    client = AdoClient(ORG, PROJ, FakeTransport(r))
    assert "error" not in fetch(100, cfg, client)
    out = trace(100, cfg, client)
    assert "error" not in out, out
    assert out["test_signal"]["fix_touched_tests"] is True
    assert out["test_signal"]["test_paths"] == ["tests/test_pay.py"]


def test_trace_signal_culprit_pr_touched_tests_true(tmp_path):
    cfg = make_cfg(tmp_path)
    r = routes_with_culprit_pr()
    r.update(diff_route(R, C1, C2, [("pay.py", "edit", None), ("tests/test_pay.py", "edit", None)]))
    client = AdoClient(ORG, PROJ, FakeTransport(r))
    assert "error" not in fetch(100, cfg, client)
    out = trace(100, cfg, client)
    assert "error" not in out, out
    assert out["test_signal"]["culprit_pr_touched_tests"] is True
